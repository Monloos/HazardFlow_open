...
import pickle
# from torchmetrics.functional.classification import average_precision as binary_average_precision
# from torchmetrics.functional.classification import auroc as binary_auroc


import lightning.pytorch as pl
# import pytorch_lightning as pl

from .drfuse_ms import DrFuseModel
...
class DrFuseTrainer(pl.LightningModule):
    def __init__(self, args, label_names, training_phase=0):
        super().__init__()
        self.model = DrFuseModel(hidden_size=args.hidden_size,
                                 ...
                                 device='cuda:'+str(args.cuda), task=args.task)
    ...
    def configure_optimizers(self):
        # CORRECT: Access lr and wd directly from self.hparams
        lr = self.hparams.lr
        wd = self.hparams.wd
        ...
        # --- stage 1 ---
            ...
            layers_to_freeze = ['time_embedding.', 'nnet.', 'final_pred_fc.']
            # layers_to_freeze = ['time_embedding.', 'nnet.']

            for name, param in self.model.named_parameters():
                if any(name.startswith(prefix) for prefix in layers_to_freeze):
        ...
        # --- stage 2 ---
            ...
            layers_to_train = ['time_embedding.', 'nnet.', 'final_pred_fc.']
            # layers_to_train = ['time_embedding.', 'nnet.']
        ...
        optimizer = optim.AdamW(trainable_params_list, lr=lr, weight_decay=wd)
        return optimizer
    ...
    def _compute_prediction_losses(self, model_output, y_gt, pairs, log=True, mode='train'):
        ...
        # loss_pred_pre = self._compute_masked_pred_loss(model_output['pre_embed'], torch.zeros_like(y_gt), ehr_mask)
        ...
        if log:
            self.log_dict({
                ...
                # f'{mode}_loss/pred_pre': loss_pred_pre.detach(),
                'step': float(self.current_epoch)
            }, on_epoch=True, on_step=False, batch_size=y_gt.shape[0])

        # return loss_pred_final, loss_pred_ehr, loss_pred_cxr, loss_pred_shared, loss_pred_pre
    ...
    def _compute_and_log_loss(self, model_output, y_gt, pairs, log=True, mode='train'):
        ...
        # loss_pred_final, loss_pred_ehr, loss_pred_cxr, loss_pred_shared, loss_pred_pre = prediction_losses
        ...
        loss_prediction = (self.hparams.lambda_pred_shared * loss_pred_shared +
                           ...
                           self.hparams.lambda_pred_cxr * loss_pred_cxr)
                            # + self.hparams.lambda_pred_shared * loss_pred_pre)
        ...
        loss_total = loss_total + self.hparams.lambda_attn_aux * loss_attn_ranking + model_output['sm']
    ...
    def on_validation_epoch_end(self):
        for name in ['final', 'ehr', 'cxr']:
            ...
            if self.task == 'mor':
                ...
                mlaps = binary_average_precision(preds, y_gt.long())
                # mlroc = multilabel_auroc(preds, y_gt.long(), num_labels=y_gt.shape[1], average=None)
                mlroc = binary_auroc(preds, y_gt.long())
            else:
                mlaps = multilabel_average_precision(preds, y_gt.long(), num_labels=y_gt.shape[1], average=None)
                mlroc = multilabel_auroc(preds, y_gt.long(), num_labels=y_gt.shape[1], average=None)
    ...
    def test_step(self, batch, batch_idx):
        x, img, y, seq_lengths, pairs = self._get_batch_data(batch)
        out = self.model(x, img, seq_lengths, pairs, 0, test_mode=True)
        with open('y_gt.pkl', 'wb') as f:
            pickle.dump(y, f)

        pred_final =  out['pred_final']
        with open('predict.pkl', 'wb') as f:
            pickle.dump(pred_final, f)
