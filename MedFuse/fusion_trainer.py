...
from models.fusion import Fusion, Fusionnew
...
class FusionTrainer(Trainer):
    ...
    def train_epoch(self):
        ...
        for i, (x, img, y_ehr, y_cxr, seq_lengths, pairs) in enumerate (self.train_dl):
            ...
            output = self.model(x, seq_lengths, img, pairs, test_mode=False)

            
            pred = output[self.args.fusion_type].squeeze()


            loss = self.loss(pred, y)+output['sm']
    ...
    def validate(self, dl, dl_type='train'):
        ...
        with torch.no_grad():
            for i, (x, img, y_ehr, y_cxr, seq_lengths, pairs) in enumerate (dl):
                ...
                img = img.to(self.device)
                output = self.model(x, seq_lengths, img, pairs, test_mode=True)
                ...
                if self.args.fusion_type != 'uni_cxr':
                    if len(pred.shape) > 1:
                        pred = pred.squeeze()
                        # pre_pred = pre_pred.squeeze()
                    
                # loss = self.loss(pred, y)+output['sm']
                loss = self.loss(pred, y)
                # loss += self.loss(pre_pred, torch.zeros_like(y))
        ...
        torch.save(all_preds, dl_type+'_out_predictions_new.pt')
    ...
    def eval(self):
        ...
        # self.model.eval()
        # ret = self.validate(self.test_dl)
        ret_train = self.validate(self.train_dl, dl_type='train')
        ret_val = self.validate(self.val_dl, dl_type='val')
        ret = self.validate(self.test_dl, dl_type='test')
        self.print_and_write(ret , isbest=True, prefix=f'{self.args.fusion_type} test', filename='results_test.txt')
        return
    def train(self):
...
class FusionTrainernew(Trainer):
    def __init__(self, 
        train_dl, 
        val_dl, 
        args,
        test_dl=None,
        train_loaded_emb = None,
        val_loaded_emb = None,
        test_loaded_emb = None
        ):

        super(FusionTrainernew, self).__init__(args)
        self.epoch = 0 
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        ...
        self.val_dl = val_dl
        self.test_dl = test_dl
        self.train_loaded_emb = train_loaded_emb
        self.val_loaded_emb = val_loaded_emb
        self.test_loaded_emb = test_loaded_emb

        self.ehr_model = LSTM(input_dim=76, num_classes=args.num_classes, hidden_dim=args.dim, dropout=args.dropout, layers=args.layers).to(self.device)
        self.cxr_model = CXRModels(self.args, self.device).to(self.device)

        self.model = Fusionnew(args, self.ehr_model, self.cxr_model, self.device ).to(self.device)
        self.init_fusion_method() # 这会调用 load_state
        # self.load_state() # 或者在这里调用
        # ...
        ...
        # 2. 然后，只解冻 lstm_fused_cls 部分的参数
        #    假设 self.model.lstm_fused_cls 是您要训练的模块
        for name, param in self.model.named_parameters():
            if 'nnet' in name or 'time_embedding' in name or 'lstm_fused_cls' in name:
        ...
        self.optimizer = optim.Adam(trainable_params, args.lr, betas=(0.9, self.args.beta_1))
    ...
    def train_epoch(self):
        ...
        outPRED = torch.FloatTensor().to(self.device)
        steps = len(self.train_dl)
        # for (i, (x, img, y_ehr, y_cxr, seq_lengths, pairs)), pred_batch in zip(enumerate(self.train_dl), self.train_loaded_emb):
        for i, (x, img, y_ehr, y_cxr, seq_lengths, pairs) in enumerate (self.train_dl):
    ...
    def validate(self, dl, loaded_emb):
        print(f'starting val epoch {self.epoch}')
        epoch_loss = 0
        epoch_loss_align = 0
        # ehr_features = torch.FloatTensor()
        ...
        with torch.no_grad():
            for i, (x, img, y_ehr, y_cxr, seq_lengths, pairs) in enumerate (dl):
            # for (i, (x, img, y_ehr, y_cxr, seq_lengths, pairs)), pred_batch in zip(enumerate(dl), loaded_emb):
                y = self.get_gt(y_ehr, y_cxr)
    ...
    def eval(self):
        ...
        # self.print_and_write(ret , isbest=True, prefix=f'{self.args.fusion_type} val', filename='results_val.txt')
        # self.model.eval()
        ret = self.validate(self.test_dl, self.test_loaded_emb)
        # ret_train = self.validate(self.train_dl, self.train_loaded_emb)
        self.print_and_write(ret , isbest=True, prefix=f'{self.args.fusion_type} test', filename='results_test.txt')
        return
    def train(self):
        print(f'running for fusion_type {self.args.fusion_type}')
        for self.epoch in range(self.start_epoch, self.args.epochs):
            self.model.eval()
            ret = self.validate(self.val_dl, self.val_loaded_emb)
            self.save_checkpoint(prefix='last')
