...
class Trainer:
    ...
    def train(
        ...
        params_to_train: Optional[List[str]] = None,    # add for two stage
    ):
        """Trains the model.
        ...
        if params_to_train:
            logger.info(f"Selectively training modules: {params_to_train}")
            
            # Freeze all parameters first
            for param in self.model.parameters():
                param.requires_grad = False
            
            # Unfreeze only the parameters of the specified modules
            params_to_optimize_list = []
            for name, param in self.model.named_parameters():
                if any(name.startswith(module_name) for module_name in params_to_train):
                    param.requires_grad = True
                    params_to_optimize_list.append(param)
            
            # Let the optimizer know only about the unfrozen parameters
            optimizer = optimizer_class(params_to_optimize_list, **optimizer_params)
        ...
        for epoch in range(epochs):
            ...
            if val_dataloader is not None:
                _, _, scores = self.evaluate(val_dataloader)
        ...
        if test_dataloader is not None:
            _, _, scores = self.evaluate(test_dataloader)
            logger.info(f"--- Test ---")
            for key in scores.keys():
                logger.info("{}: {:.4f}".format(key, scores[key]))
    ...
    def inference(self, dataloader, additional_outputs=None,
        ...
        for data in tqdm(dataloader, desc="Evaluation"):
            self.model.eval()
            with torch.no_grad():
                output = self.model(**data, test_mode=True)
                loss = output["loss"]
        ...
        if self.model.mode is not None:
            ...
            # for i in y_true_all:
                ...
                # else:
                    # cnt1 += 1
            # print("cnt0, cnt1", cnt0, cnt1)
        return y_true_all, y_prob_all, scores
