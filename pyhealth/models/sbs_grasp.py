...
class SBSGRASP(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ''' This is initialization of HazardFlow '''
        ...
        self.T = 2    # hazard escaltion steps
        self.T_p = 2  # hazard reduction steps
        self.EPS = 1.e-5
    ...
    def sample(self, x_0, batch_size=64, test_mode=False):
        ...
        for t in ts[1:]:
            ...
            x_t = x_t - delta_t * u
            # if test_mode==True:
            #     with open('23sample'+str(cnt)+'.pkl', 'wb') as f:
            #         pickle.dump(x_t, f)
            #     cnt+=1
            # print("x_t, x_t.shape", x_t, x_t.shape)
        # x_t = torch.tanh(x_t)
        return x_t
    ...
    def score_based_loss(self, patient_emb, aggr='mean', test_mode=False):
        ...
        # hazard_rate = self.sigmoid(hazard)
        # force = 1 - torch.exp(-hazard_rate)
        # pre_embed = self.sample(patient_emb)
        final_embed = self.sample(patient_emb)
        pre_embed = self.re_sample(patient_emb)
        # final_embed = patient_emb-self.sigmoid(hazard)        
        # final_embed = patient_emb+force        
        ...
        # ce_loss = self.get_loss_function()(hazard_pred, y_true)

        # return sm_loss + ce_loss
        # return sm_loss, hazard
        return sm_loss, final_embed, pre_embed
    ...
    def forward(self, **kwargs) -> Dict[str, torch.Tensor]:
        ...
        '''We plug HazardFlow here'''
        sm_loss, final_embed, pre_embed = self.score_based_loss(patient_emb=patient_emb,test_mode=False)
        # (patient, label_size)
        logits = self.fc(final_embed)
        logits_old = self.fc(pre_embed)
        # obtain y_true, loss, y_prob
        y_true = self.prepare_labels(kwargs[self.label_key], self.label_tokenizer)
        y_old = torch.zeros_like(y_true)

        loss = self.get_loss_function()(logits, y_true) + sm_loss + self.get_loss_function()(logits_old, y_old)
